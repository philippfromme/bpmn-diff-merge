...
export default function App() {
  ...
  const [ remoteDiff, setRemoteDiff ] = React.useState(null);
  ...
  const [ mergedXML, setMergedXML ] = React.useState(window.baseXml);
  ...
  useEffect(() => {
    findConflicts(window.baseXml, window.localXml, window.remoteXml).then(({ conflicts, localDiff, remoteDiff }) => {
      setConflicts(conflicts);
      setLocalDiff(localDiff);
      setRemoteDiff(remoteDiff);
  ...
  useEffect(() => {
    if (!mergedViewer || !localDiff || !remoteDiff || !conflicts) return;

    merge(window.baseXml, localDiff, remoteDiff, conflicts, conflictDecisions).then(newXML => {
      console.log('Setting merged XML, is different?', newXML !== mergedXML);

      setMergedXML(newXML);
    });

  }, [ localDiff, remoteDiff, conflicts, conflictDecisions, mergedViewer, mergedXML, setMergedXML ]);
      ...
      <div className="main">
        ...
        <div className="diagrams">
          ...
          <div className="bottom">
            ...
            <Diagram xml={mergedXML} previousXml={window.baseXml} viewer={mergedViewer} />
          </div>
        </div>
      </div>
    </>
...
function Diagram(props) {
  ...
  useEffect(() => {
    if (!ref.current || !viewer || !xml) return;

    viewer.attachTo(ref.current);

    console.log('Importing XML:', xml.substring(0, 100) + '...');
    ...
    viewer.importXML(xml).then(() => {
      ...
      if (previousXml) {
        renderDiff(previousXml, xml, viewer);
      }
    }).catch(err => {
...
async function renderDiff(before, after, viewer) {
  const diffResult = await diffXML(before, after);
...
`,
  ...
  for (const element of Object.values(layoutChangedElements)) {
    addMarkers(element, ['layout-changed'], viewer);
  }

  for (const element of Object.values(addedElements)) {
    addMarkers(element, ['added'], viewer);
  }

  for (const { element} of Object.values(changedElements)) {
    addMarkers(element, ['changed'], viewer, 2000);
  }
}